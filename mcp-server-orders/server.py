    ...
    return [
        {
            ...
            "description": "Notify pending orders for a product",
            "input_schema": {
                "type": "object",
                "properties": {
                    "product_id": {"type": "integer", "description": "Product ID to notify"}
                },
                "required": ["product_id"]
            }
        }
        ...
        if tool == "orders.notifyPending":
            ...
            try:
                ...
                return respond(id_, {
                    "status": "notified",
                    "product_id": pid,
                    "message": f"Pending orders notification sent for product {pid}"
                })
            except Exception as e:
                return respond(id_, error=f"Redis error: {str(e)}")
